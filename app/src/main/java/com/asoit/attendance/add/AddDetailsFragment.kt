...
class AddDetailsFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val BranchOptions = arrayOf("SELECT BRANCH","CE","IT","ME","EC","CIVIL","AERONAUTICAl")
        val SemOptions = arrayOf("SELECT SEMESTER","1","2","3","4","5","6","7","8")
        val LectureOptions = arrayOf("SELECT LECTURE","1","2","3","4","5","6","Lab")

        branchSpinner.adapter = ArrayAdapter<String>(this.requireContext(),android.R.layout.simple_list_item_1,BranchOptions)
        semSpinner.adapter = ArrayAdapter<String>(this.requireContext(),android.R.layout.simple_list_item_1,SemOptions)
        lectureSpinner.adapter = ArrayAdapter<String>(this.requireContext(),android.R.layout.simple_list_item_1,LectureOptions)
       ...
       btn_save.setOnClickListener {
          ...
          _total = total.text.toString()
           ...
           {
               var action =  AddDetailsFragmentDirections.actionAddDetailsFragmentToAbsentPresentFragment(_date,_branch,_lecture,_sem,_total,_note)
